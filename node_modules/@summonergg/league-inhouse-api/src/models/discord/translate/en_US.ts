...
export const en_US: Translater = (msg: DiscordMessage) => {
  return (id, args?) => {
    ...
    var res: Record<number, string> = {
      ...
      201: `\`${args?.region}\` is not a valid region`,
      202: "name cannot be empty",
      203: "league characters have no more than 16 characters",
      204: `The summoner name \`${args?.name}\` does not exist`,
      205: `The summoner name \`${args?.name}\` is already being used by someone else`,
